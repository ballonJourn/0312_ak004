...
static void _notify_call_num_cb(bt_call_num_e num) {
	Mutex::Autolock _l(_s_cb_mutex);
	CB_FOREACH(_s_cb_set, bt_cb_t *, call_num_cb, num);
}
...
static void _proc_hfp_state(const char *data, int len) {
	...
	case '4':
		...
		if (lk::get_lylink_type() == LINK_TYPE_NULL || lk::get_lylink_type() == LINK_TYPE_WIFIAUTO) {
...
static void _proc_call_outgoing(const char *data, int len) {
	...
	if((lk::get_lylink_type() == LINK_TYPE_WIFIAUTO || lk::get_lylink_type() == LINK_TYPE_USBAUTO)
			&& _s_bt_info.auto_open == false){
		return;
	}
	if (lk::get_lylink_type() == LINK_TYPE_WIFIAUTO || lk::get_lylink_type() == LINK_TYPE_USBAUTO) {
		entry_lylink_ftu();
		...
		_notify_call_num_cb(E_BT_CALL_NUM);
	}
}
...
//	LOCK_OP(_s_info_mutex, _s_bt_info.call_start_time = LLONG_MAX);
	...
	// 结束通话状态——>音量恢复
	if (_s_bt_info.calling) {
		_s_bt_info.calling = false;
		audio::handle_phone(E_AUDIO_TYPE_BT_PHONE, false);
	}

	_s_bt_info.call_num.clear();
//	audio::set_system_vol(audio::get_system_vol());			// 通话状态强制结束——>音量恢复
	//  LOGD("[DEBUG] hangup - music_playing: %d, audio_type: %d, bt_is_mute: %d",
	// 	_s_bt_info.music_playing, audio::get_audio_type(), _s_bt_info.bt_is_mute);
    
    // // if (_s_bt_info.music_playing) {
    // //     LOGD("[DEBUG] ");
	// // 	set_bt_mute(false, true);  // 取消静音并切换音源
    // // }
	// set_bt_mute(false, true);  // 取消静音并切换音源
}
...
bool call_vol(float vol) {
	...
	uint8_t target_val = vol_map[level];
	snprintf(cmd, sizeof(cmd), "%s%d", BT_CMD_CALL_VOL, (int)(target_val));
	return _send_cmd(cmd);
}
