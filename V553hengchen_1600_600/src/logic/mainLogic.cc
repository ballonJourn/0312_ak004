...
// incell 屏幕触摸键值定义（需要根据实际日志确认后修改）
#define INCELL_TOUCH_KEYCODE_1    0x145  // BTN_TOUCH (示例值)
#define INCELL_TOUCH_KEYCODE_2    0x14A  // BTN_TOOL_FINGER (示例值)
#define INCELL_TOUCH_KEYCODE_3    0x14B  // BTN_TOOL_DOUBLETAP (示例值)
...
static void _update_time_display() {
    ...
    if (!sys::setting::is_time_format_24h()) {
        ...
        mampmTextViewPtr->setText(ampmStr.c_str());
        mampmTextViewPtr->setVisible(true);
    } else {
        // 24小时制
        sprintf(time, "%02d:%02d", hour, t->tm_min);
        mampmTextViewPtr->setVisible(false);
    }
...
static void _lylink_callback(LYLINKAPI_EVENT evt, int para0, void *para1) {
	...
	case LYLINK_LINK_ESTABLISH:
		...
		base::runInUiThreadUniqueDelayed("restart record", []() {
			if (sys::setting::get_resolution_mode() == E_RESOLUTION_MODE_2_5K_720P) {
	...
	case LYLINK_LINK_DISCONN:
		...
		mActivityPtr->registerUserTimer(LINK_CONNECT_TIP_TIMER, 0); // @suppress("无效参数")
		base::runInUiThreadUniqueDelayed("restart record", []() {
			if (sys::setting::get_reduce_resolution()) {
				restartRecorder(true);
			}
		}, 1000);
		break;
...
static void _event_mode_cb(event_mode_e mode) {
	// 移除了HOME模式的window切换逻辑
	// 如果需要其他事件模式处理，可以在这里添加
}
...
protected:
	virtual void onSlidePageChange(ZKSlideWindow *pSlideWindow, int page) {
		...
		case 2:
			...
			break;
		case 3:
			mStatusRadioGroupPtr->setCheckedID(ID_MAIN_RadioButton3);
			break;
		default:
			break;
		}
...
/**
 * 判断是否是 incell 屏幕触摸产生的按键事件
 * 参数: keyCode - 按键码
 * 返回: true - 是触摸事件，false - 不是触摸事件
 */
static inline bool _is_incell_touch_key(int keyCode) {
	// 添加调试日志，用于确认 incell 触摸的实际键值
	// 部署到 incell 屏幕后，观察日志输出，确认具体的键值
	// LOGD("[main] KeyEvent: mKeyCode=0x%X (%d)", keyCode, keyCode);

	// 常见的触摸相关键值 (需要根据实际日志确认后调整)
	return (keyCode == INCELL_TOUCH_KEYCODE_1 ||  // BTN_TOUCH (0x14A)
	        keyCode == INCELL_TOUCH_KEYCODE_2 ||  // BTN_TOOL_FINGER
	        keyCode == INCELL_TOUCH_KEYCODE_3);   // BTN_TOOL_DOUBLETAP
}
...
protected:
	virtual bool onKeyEvent(const KeyEvent &ke) {
	  // 添加调试日志，确认 incell 触摸的键值
	  LOGD("[main] KeyEvent: mKeyCode=0x%X (%d), mKeyStatus=%d",
	       ke.mKeyCode, ke.mKeyCode, ke.mKeyStatus);

	  // 过滤 incell 屏幕触摸事件，避免触发关机/息屏逻辑
	  if (_is_incell_touch_key(ke.mKeyCode)) {
	    LOGD("[main] Ignored incell touch event: keyCode=0x%X", ke.mKeyCode);
	    return true;  // 拦截事件，不再处理
	  }
    ...
    case MountMonitor::E_MOUNT_STATUS_MOUNTED:
      ...
      if (ENABLE_AUTO_RECORD && !isTesting()) {
    	  try {
    		  ...
    		  LOGE_TRACE("e.what() = %s", e.what());
    		  break;
    	  } catch (...) {
    		  LOGE_TRACE("unknown error !!!");
    		  break;
    	  }
      }
      if (base::exists(
...
void onReverseCameraChanged(ReverseCameraState state) {
  ...
  if (state == REVERSE_CAMERA_STATE_UNDETECTED) {
    ...
    if (strcmp(EASYUICONTEXT->currentAppName(), "mainActivity") == 0) {
...
//  sys::hw::set_light_auto_switch(sys::setting::is_light_state());
  ...
  // 长按关机后开机判断是否打开蓝牙
  if (sys::setting::get_bt_state()) {
	  bt::power_on();
	  sys::setting::set_bt_state(false);
  }
}
...
//  setupCamera(VIEW_TYPE_REAR, true);
  ...
  mdockBtButtonPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
  mweekTextViewPtr->setLongMode(ZKTextView::E_LONG_MODE_DOTS);
...
static bool onUI_back() {
	...
	// 移除了window切换逻辑，直接返回false让系统处理返回事件
	return false;
...
static bool onUI_Timer(int id) {
	...
	case LINK_CONNECT_TIP_TIMER: {
		...
		// 使用appSlideWindow设置文本
		mappSlideWindowPtr->setItemText(0, cp_tip);  // CP按钮索引0
		mappSlideWindowPtr->setItemText(1, aa_tip);  // AA按钮索引1
		return false;
	}
	case BT_MUSIC_ID3:
		if(bt::music_is_playing()){
			bt::query_bt_music_info();
		}
		// 开机获取mcu版本号进行保存
    ...
    case TIMER_START_CAMERA:
      ...
      try {
        auto view = sys::has_reverse_camera() ?
            (ViewType)sys::setting::get_dvr_view_type() : VIEW_TYPE_FRONT;
        setupCamera(VIEW_TYPE_FRONT_MINI, true);
		// 解决dvr回到主界面黑屏的问题
      } catch (mpi::Exception& e) {
        LOGE_TRACE("%s", e.what());
      }
...
static bool onmainActivityTouchEvent(const MotionEvent &ev) {
	// 移除了window切换的触摸事件处理逻辑
	// appSlideWindow会自动处理滑动事件
...
static bool onButtonClick_LinkButton(ZKButton *pButton) {
    ...
    if (lk::is_connected()) {
		...
		if (E_LINK_MODE_BLUEMUSIC == sys::setting::get_link_mode()) {
...
static void onSlideItemClick_appSlideWindow(ZKSlideWindow *pSlideWindow, int index) {
    LOGD(" onSlideItemClick_appSlideWindow %d !!!\n", index);
	switch(index) {
	case 0:
	    LOGD(" ButtonClick DVRButton !!!\n");
	    OPEN_APP("Dvr");
	    break;
	case 1:
	    LOGD(" ButtonClick CPButton !!!\n");
...
static bool onButtonClick_sys_home_Button(ZKButton *pButton) {
    LOGD(" ButtonClick sys_home_Button !!!\n");
    return false;
}
...
static bool onButtonClick_weekTextView(ZKButton *pButton) {
    LOGD(" ButtonClick weekTextView !!!\n");
    return false;
}
