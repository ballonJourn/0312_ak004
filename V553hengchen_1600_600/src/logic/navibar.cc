...
void NavibarSetMcuUpgradeMask(bool visible) {
    if (mTextView3Ptr == NULL) {
        return;
    }
    mTextView3Ptr->setVisible(visible);
    LOGD("[navibar] MCU upgrade mask set to %s", visible ? "visible" : "hidden");
}
