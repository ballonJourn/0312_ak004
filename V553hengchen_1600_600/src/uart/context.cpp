...
const std::string hexToAscii(const std::string& hexStr) {
    std::string result;
    std::string hexDigits;

    // 提取所有十六进制数字，忽略空格
    for (char c : hexStr) {
        if (std::isxdigit(c)) { // 检查是否为十六进制数字
            hexDigits += c;
        }
    }
    if (hexDigits.length() % 2 != 0) {
    	LOGE_TRACE("%s,%d", hexDigits.c_str(), hexDigits.length());
        return result;
    }
    for (size_t i = 0; i < hexDigits.length(); i += 2) {
        std::string byteString = hexDigits.substr(i, 2);
        char byte = static_cast<char>(std::stoul(byteString, nullptr, 16));
        result += byte;
    }

    LOGE_TRACE("%s",result.c_str());
    return result;
}
...
//	}
	...
	case CMD_QUERY_VER:				// mcu版本号
		...
		sys::setting::set_mcu_version(hexToAscii(str));
		break;
	case CMD_FM_SW:
